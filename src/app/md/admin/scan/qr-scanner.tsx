...
/**
 * Native getUserMedia QR scanner for the MD admin scan-to-verify page.
 *
 * Same approach as the BTS scanner: live camera viewport via
 * getUserMedia (rear camera preferred) using the native BarcodeDetector
 * API where available, with a manual Application ID entry fallback.
 *
 * On a successful read, redirects to /md/verify?aid=<id>.
 */
export function MdQrScanner() {
  ...
  function handleDetectedValue(raw: string) {
    ...
    setDetectedHint(aid);
    setTimeout(() => {
      router.push(`/md/verify?aid=${encodeURIComponent(aid)}`);
    }, 350);
  }
  ...
  return (
    <div className="space-y-6">
      {/* Camera viewport */}
      <div className="md-animate-fade-in-up rounded-2xl border border-amber-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-bold text-amber-800">Camera Scanner</h2>
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border-2 border-amber-300 bg-gray-900">
          <div className="relative aspect-square w-full sm:aspect-[4/3]">
            ...
            {cameraState !== "live" && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-6 text-center">
                ...
                {cameraState === "idle" && (
                  <>
                    <CameraIcon className="h-12 w-12 text-amber-300" />
            ...
            {cameraState === "live" && (
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                <div className="h-48 w-48 rounded-xl border-2 border-amber-400/80 shadow-[0_0_0_9999px_rgba(0,0,0,0.35)]">
                  <div className="md-scan-line h-0.5 w-full bg-amber-300" />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {cameraState === "live" ? (
            <button
              type="button"
              onClick={stopCamera}
              className="inline-flex min-h-[44px] items-center justify-center rounded-lg border border-gray-300 bg-white px-5 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors"
            >
              Stop camera
            </button>
          ) : (
      ...
      <div className="md-animate-fade-in-up rounded-2xl border border-amber-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-bold text-amber-800">Manual Entry</h2>
        <p className="mt-1 text-sm text-gray-600">
          If the camera isn&rsquo;t available, type or paste the Application ID (e.g.{" "}
          <code className="font-mono">MD-260806-ABC123</code>).
        </p>
