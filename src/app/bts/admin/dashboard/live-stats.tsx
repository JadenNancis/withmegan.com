...
export function LiveStats({ site }: Props) {
  ...
  const accentBarFrom = site === "bts" ? "from-cyan-400" : "from-amber-400";
  const accentBarTo = site === "bts" ? "to-blue-500" : "to-orange-500";
  const accentRing = site === "bts" ? "ring-cyan-500/40" : "ring-amber-500/40";
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Progress bar */}
      <section
        className={`rounded-2xl border border-slate-700 bg-slate-800/60 p-6 shadow-xl ring-2 ${accentRing}`}
      >
...
function StatTile({
  ...
  return (
    ...
    >
      ...
      <p className={`mt-2 text-5xl font-bold tabular-nums ${accentText}`}>
        {value}
      </p>
    </div>
  );
