  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="inline-flex items-center rounded-full border border-white/20 bg-brand-950/40 backdrop-blur px-3 py-1.5 text-sm text-neutral-100">
          {lastUpdated
            ? `Updated ${lastUpdated.toLocaleTimeString("en-TT")}`
            : "Loading…"}
          {error && <span className="ml-2 text-red-400">· {error}</span>}
        </p>
        <p className="inline-flex items-center rounded-full border border-white/20 bg-brand-950/40 backdrop-blur px-3 py-1.5 text-2xl font-mono font-semibold tabular-nums text-white">
      ...
      {/* Progress bar */}
      <section
        className={`rounded-2xl border border-white/25 bg-brand-950/50 backdrop-blur-md p-6 shadow-xl ring-2 ${accentRing}`}
      >
        <div className="flex items-end justify-between">
          <h2 className="text-xl font-semibold text-white [text-shadow:0_1px_4px_rgba(0,0,0,0.65)]">
            {label} progress
          </h2>
        ...
        <div className="mt-4 h-8 w-full overflow-hidden rounded-full bg-white/15 backdrop-blur-sm">
          <div
            className={`h-full bg-gradient-to-r ${accentBarFrom} ${accentBarTo} transition-all duration-700 ease-out shadow-[0_0_12px_rgba(34,211,238,0.5)]`}
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      {/* Community breakdown */}
      <section className="rounded-2xl border border-white/25 bg-brand-950/50 backdrop-blur-md p-6 shadow-xl">
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-white/10">
                {stats.byCommunity.map((c) => {
                  ...
                  return (
                    <tr key={c.community} className="hover:bg-white/[0.08] transition-colors">
...
function StatTile({
  ...
  return (
    <div
      className={`rounded-2xl border border-white/25 bg-brand-950/50 backdrop-blur-md p-6 shadow-xl ring-2 ${accentRing} hover:shadow-2xl transition-shadow`}
    >
      <p className="text-xs font-semibold uppercase tracking-widest text-neutral-200">
        {label}
      </p>
      <p className={`mt-3 text-5xl font-bold tabular-nums ${accentText} [text-shadow:0_2px_8px_rgba(0,0,0,0.4)]`}>
        {value}
      </p>
    </div>
  );
