...
import { GuardianStep } from "@/app/bts/register/steps/guardian-step";
import { DependentsStep, type DependentForm, emptyDependent } from "@/app/bts/register/steps/dependents-step";
import { ReviewStep, type SubmitResult } from "@/app/bts/register/steps/review-step";
import { SuccessCard } from "@/app/bts/register/success-card";
import { InterestCard } from "@/app/bts/register/interest-card";
import { useWizardDraft } from "@/app/bts/register/draft";
import { OTHER_LOCATION_VALUE } from "@/lib/bts-locations";
import { OTHER_SCHOOL_VALUE, OTHER_GRADE_VALUE } from "@/lib/bts-schools";
...
export interface BtsRegisterWizardProps {
  /** "public" = site Register tab (draft autosave on). "walkin" = admin Walk-in (stateless kiosk form). */
  mode: "public" | "walkin";
  /** Where the footer back-link and success card go. */
  backHref?: string;
  backLabel?: string;
  backHint?: string;
...
/**
 * The three-step registration wizard shared by the public Register tab and
 * the admin Walk-in counter. Walk-in mode disables draft persistence (a
 * kiosk must never resume the previous person's form) and points the exit
 * links back to the admin area.
 */
export function BtsRegisterWizard({
  mode,
  backHref,
  backLabel,
  backHint,
  registerAnotherLabel,
}: BtsRegisterWizardProps) {
  const isWalkin = mode === "walkin";
  const draft = useWizardDraft<WizardState>(
    isWalkin ? "bts-walkin-draft" : "bts-draft-v1",
    initialState,
    { enabled: !isWalkin },
  );
  ...
  const exitHref = backHref ?? (isWalkin ? "/bts/admin" : "/bts");
  const exitLabel = backLabel ?? (isWalkin ? "Back to admin" : "Back to home");
  const exitHint = backHint ?? (isWalkin ? "Return to the admin dashboard" : "Return to the site");
  const anotherLabel =
    registerAnotherLabel ?? (isWalkin ? "Register another walk-in" : "Register another family");
  ...
  function validateCurrentStep(): string | null {
    ...
    if (state.step === 1) {
      for (let i = 0; i < state.dependents.length; i++) {
        ...
        if (!d.gradeLevel.trim()) return `Child/Student ${i + 1}: grade or form is required.`;
    ...
    try {
      const payload = {
        ...
        dependents: state.dependents.map((d) => ({
          ...
          gradeLevel:
            d.gradeLevel === OTHER_GRADE_VALUE ? d.manualGradeLevel : d.gradeLevel,
  ...
  if (waitlistCommunity) {
    return (
      <InterestCard
        community={waitlistCommunity}
        onRegisterAnother={resetForAnother}
        backHref={exitHref}
        backLabel={exitLabel}
        registerAnotherLabel={anotherLabel}
      />
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Draft resume banner (public mode only — walk-ins never leave drafts) */}
      {draft.hadDraft && (
