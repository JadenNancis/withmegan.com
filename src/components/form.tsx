...
const inputBase =
  "w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-transparent focus:ring-2 focus:ring-blue-500 focus:outline-none";
...
export function SubmitButton({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <button
      type="submit"
      className={cn(
        "rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors",
        className,
      )}
    >
      {children}
